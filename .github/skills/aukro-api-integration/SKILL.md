...
3. **Skill asset (prefer when using this skill):** `.github/skills/aukro-api-integration/openapi.yaml`
...
3. **If the official changelog has a newer date**
   ...
   - **Save** it as the skill asset: `.github/skills/aukro-api-integration/openapi.yaml` (overwrite existing). Use the Write tool or a terminal command writing the response body to that path.
   ...
   - **Validate docs against the updated spec:** run `node .github/skills/aukro-api-integration/scripts/validate-skill-docs.mjs`. If it reports deprecated/missing endpoint docs or schema mismatches, update SKILL.md/reference.md before finishing.
